...
static int get_default_device(fprintd_ctx *ctx)
{
    ...
    if (!msg) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: failed to create GetDefaultDevice message");
        return -1;
    }
    ...
    if (!reply) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: GetDefaultDevice failed: %s",
               err.message);
        dbus_error_free(&err);
        return -1;
    }
    ...
    if (!dbus_message_get_args(reply, &err,
                               ...
                               DBUS_TYPE_INVALID)) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: GetDefaultDevice bad reply: %s",
               err.message);
        dbus_error_free(&err);
    ...
    if (!ctx->device_path) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: strdup failed for device path");
        return -1;
    }
...
static void add_verify_signal_match(fprintd_ctx *ctx)
{
    ...
    if (dbus_error_is_set(&err)) {
        syslog(LOG_AUTH | LOG_WARNING,
               "pam_fprintd_tty: add_match failed: %s", err.message);
        dbus_error_free(&err);
    }
...
int fprintd_open(fprintd_ctx *ctx)
{
    ...
    if (!ctx->conn) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: cannot connect to system bus: %s",
               err.message);
        dbus_error_free(&err);
        return -1;
    }
...
int fprintd_has_enrolled_prints(fprintd_ctx *ctx, const char *username)
{
    ...
    if (!reply) {
        ...
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: ListEnrolledFingers failed: %s",
               err.message);
        dbus_error_free(&err);
        return -1;
    }
...
int fprintd_verify_start(fprintd_ctx *ctx, const char *username)
{
    ...
    if (!ctx->claimed) {
        ...
        if (!reply) {
            syslog(LOG_AUTH | LOG_ERR,
                   "pam_fprintd_tty: Claim(%s) failed: %s",
    ...
    if (!reply) {
        syslog(LOG_AUTH | LOG_ERR,
               "pam_fprintd_tty: VerifyStart failed: %s",
               err.message);
        dbus_error_free(&err);
        remove_verify_signal_match(ctx);
        return -1;
        ...
        if (dbus_message_is_signal(msg,
            ...
            if (dbus_message_get_args(msg, &err,
                ...
                syslog(LOG_AUTH | LOG_DEBUG,
                       "pam_fprintd_tty: VerifyStatus: %s (done=%d)",
                       status, (int)done);
